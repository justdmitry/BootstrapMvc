namespace BootstrapMvc.Forms
{
    ...
    public class ValidationSummary<TModel> : ValidationSummary
    {
        ...
        protected override void WriteSelf(System.IO.TextWriter writer)
        {
            ...
            if (haveErrors)
            {
                using (Helper.CreateWriter<Alert, AnyContent>(this).Type(Color8.DangerRed).BeginContent(writer))
                {
            ...
            if (haveWarnings)
            {
                using (Helper.CreateWriter<Alert, AnyContent>(this).Type(Color8.WarningOrange).BeginContent(writer))
                {
